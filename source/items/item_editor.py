...
from source.items.item_loader import add_data
from source.items.itemdata_editor import input_item_data
...
async def get_item_data_info(message, item_id):
    ...
    message_text = "📋 Тип товара: <b>{type}</b>\n📦 Данные:\n\n<b>{data}</b>" \
        .format(type=type_data,
                data=item_type[1])
    keyboard = types.InlineKeyboardMarkup()
...
async def load_value(message, state):
    ...
    if param == "name":
        # название
        database.edit_name(item_id, value)
    elif param == "desc":
        # описание
        database.edit_desc(item_id, value)
    elif param == "pic":
        # картинка
        database.edit_pic(item_id, value)
    elif param == "price":
        # цена
        database.edit_price(item_id, value)
