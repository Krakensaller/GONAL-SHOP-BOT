...
def create_qiwi_link(amount, comment):
    ...
    if payment_type == "number":
        # оплата по номеру
        payment_form["link"] = f"https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={qiwi_data[1]}&amountInteger={amount}&amountFraction=0&extra%5B%27comment%27%5D={comment}&currency=643&blocked%5B0%5D=sum&blocked%5B1%5D=comment&blocked%5B2%5D=account"

        payment_form["key"] = "Номер"
        payment_form["value"] = qiwi_data[1]
    elif payment_type == "nickname":
        # оплата по нику
        payment_form["link"] = f"https://qiwi.com/payment/form/99999?extra%5B%27account%27%5D={qiwi_data[3]}&amountInteger={amount}&amountFraction=0&currency=643&blocked[0]=account&extra%5B%27accountType%27%5D=nickname"
...
def check_qiwi_payment(amount, comment):
    ...
    for i in range(len(last_payments)):
        ...
        amount_pay = str(last_payments[i]["sum"]["amount"])
        currency_pay = str(last_payments[i]["sum"]["currency"])

        if comment == comment_pay and amount_pay == str(amount) and currency_pay == "643":
            return True
