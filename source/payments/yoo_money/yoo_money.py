...
from source.payments.yoo_money.yoo_money_params import yoomoney_auth, generate_token
...
async def redirect_uri(message: types.Message, state: FSMContext):
    ...
    id = message.text
    if is_const(id):
        await message.answer("❗️ Введите корректный <b>client_id</b> ❗️")
        return
...
async def authorize_url(message: types.Message, state: FSMContext):
    ...
    uri = message.text
    if is_const(uri):
        await message.answer("❗️ Введите корректный <b>redirect_uri</b> ❗️")
        return
    ...
    auth_url = yoomoney_auth(data['client_id'], data['redirect_uri'])

    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton(text="💻 Активация", url=auth_url))

    await message.answer("❗ Перейдите по ссылке, подтвердите данные и скопируйте полученную ссылку после "
                         "переадресации\n "
                         "❗ Время действия полученной ссылки <b>1 минута</b>",
...
async def authorize_payment(message: types.Message, state: FSMContext):
    ...
    url = message.text
    if is_const(url):
        await message.answer("❗️ Некорректная ссылка ❗️")
        return
